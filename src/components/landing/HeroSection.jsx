    ...
    <section id="hero" className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 pt-28 pb-32 md:pb-32 lg:pb-32 w-full">
        <div className="max-w-3xl">
          ...
          >
            <div className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-primary/10 backdrop-blur-md border border-primary/20">
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className="w-3 h-3 text-accent fill-accent" />
                ))}
              </div>
              <span className="text-primary text-sm font-medium font-body ml-1">Entreprise familiale · Depuis 2016</span>
            </div>
          </motion.div>
          ...
          <motion.h1
            ...
            className="font-heading font-extrabold text-5xl md:text-6xl lg:text-[5.5rem] text-primary leading-[1.05] mb-6"
          >
            D'un seau
            <br />
          ...
          <motion.p
            ...
            className="text-lg md:text-xl text-foreground/70 leading-relaxed mb-4 max-w-xl font-body"
          >
            Nettoyer un espace, c'est prendre soin des personnes qui y vivent et travaillent. 
            Professionnalisme, confiance et humanité depuis plus de 9 ans.
          </motion.p>
          ...
          >
            {['Devis gratuit sous 24h', 'Équipe formée & assurée', 'Produits certifiés éco'].map((item) => (
              <div key={item} className="flex items-center gap-2">
                ...
                <span className="text-foreground/70 text-sm font-body">{item}</span>
              </div>
            ))}
          </motion.div>
          ...
          >
            ...
            <a
              href="#histoire"
              className="border border-primary/30 hover:bg-primary/10 text-primary px-8 py-4 rounded-full text-base font-medium transition-all duration-300 text-center"
            >
              Notre Histoire
            </a>
          </motion.div>
      ...
      >
        <div className="bg-white/80 backdrop-blur-xl border-t border-primary/10">
            ...
            <div className="grid grid-cols-4 divide-x divide-primary/10">
              {stats.map((s) => (
                <div key={s.label} className="flex flex-col items-center py-2.5 px-2">
                  ...
                  <span className="text-foreground/50 text-[10px] md:text-xs font-body text-center leading-tight mt-0.5">{s.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      >
        <span className="text-primary/40 text-xs font-body tracking-widest uppercase rotate-90 origin-center mb-4">Scroll</span>
